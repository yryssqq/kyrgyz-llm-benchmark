...
translation score computed on a test set that was itself produced by machine
translation. Such a score conflates two systems: it rewards the model for the parts
...
TODO. This section is a stub and must be written from a literature review. Cover:
(i) multilingual LLM benchmarks and their language coverage; (ii) Turkic-language NLP
resources and benchmarks (for example the Universal Dependencies Kyrgyz treebanks and
recent Turkic evaluation suites); (iii) morphological evaluation and probing for
agglutinative languages, including the SIGMORPHON inflection tasks; (iv) rule-based
and synthetic data generation for low-resource languages; and (v) the Apertium
Kyrgyz description used here. Position the contribution as evaluating the
\emph{production} of morphology under a guessing-proof protocol, rather than
translation quality.
...
\paragraph{Models.} We evaluate \texttt{gpt-4o} and \texttt{gpt-4o-mini} on the core
benchmark. The fine-tuning study additionally uses the open models
\texttt{Qwen2.5-0.5B-Instruct} and \texttt{Qwen2.5-3B-Instruct}. TODO: fix model
version strings and access dates, and add further frontier models (for example
Gemini and Claude) to show the dissociation is not specific to one provider.
...
early version of our own rule, which an independent finite-state description of Kyrgyz
confirmed, after which the rule was corrected. We regard this as evidence for, not
against, the reliability of the final engine.

\subsection{Stem selection}

Stems are drawn from three independent resources: the Apertium Kyrgyz morphological
dictionary, the Universal Dependencies Kyrgyz treebanks, and Wiktionary. A stem is
kept only if at least two of the three attest it, which removes typos and one-off
...
We fine-tune with LoRA on the rule-generated training pairs and evaluate two ways: a
...
different abilities. Generation isolates whether the morphological rules have been
