...
% NeurIPS 2026 workshop (LP4FM) submission -- double-blind, anonymous.
% Requires the official neurips_2026.sty from the workshop / NeurIPS site.
% No option => anonymous "under review" mode (prints "Anonymous Author(s)").
% For camera-ready, switch to \usepackage[final]{neurips_2026} and de-anonymise.
...
% Anonymous for double-blind review. NeurIPS style prints "Anonymous Author(s)"
% in submission mode; this block is a placeholder for the camera-ready.
\author{%
  Anonymous Author(s)\\
  Affiliation\\
...
I introduce a native-speaker-authored benchmark of 64 four-option items across eight
...
result is tested against a 25\% random baseline. Evaluating two frontier models I
...
(gpt-4o 82.7\%, gpt-4o-mini 65.7\%). I then show the gap is learnable rather than
...
0\% to 77\% at producing inflected forms for held-out words. I release the benchmark,
...
I build such a benchmark for Kyrgyz. Every item is written and verified by a native
...
culturally specific content. My benchmark is deliberately narrow and native: one
...
subject-matter knowledge expressed in these languages; I instead isolate grammatical
...
Universal Dependencies treebanks \cite{nivre2020ud}, which I use as one source of
attested nouns, and the Apertium finite-state description
\cite{forcada2011apertium,khanna2021apertium}, whose morphological rules I use both
as a stem source and as an independent check on my own rule engine.
...
My generation task is in that tradition, but the object of study is different: rather
than building the best inflection system, I ask whether a general-purpose LLM can
...
\cite{anastasopoulos2019inflection}. My rule engine is a deterministic instance of
the same idea: because Kyrgyz nominal morphology is rule-governed, I can generate
correct training pairs for arbitrarily many attested stems rather than sampling them,
and I use them to teach the phenomenon rather than to win a shared task.
...
does not probe morphology directly. For lexical resources I draw on Wiktionary via
...
To confirm that the answer keys are correct and unambiguous, two native-speaker Kyrgyz
language teachers, neither of whom authored the items, independently answered all 64
core-benchmark questions without access to the key. Both selected the keyed option on
every item: agreement with the key was 64/64 (100\%) for each teacher, and agreement
between the two annotators was likewise complete, giving Cohen's $\kappa = 1.0$.
Perfect agreement is the desired outcome here rather than a suspicious one: the items
are designed so that the difficulty lies in the language as seen by a model, not in any
ambiguity for a fluent speaker, and unanimous native agreement confirms that the keys
are not in dispute.
...
\paragraph{Models.} I evaluate the OpenAI models \texttt{gpt-4o} and
\texttt{gpt-4o-mini} \cite{openai2023gpt4}, accessed through the API by their default
aliases rather than a pinned dated snapshot. The core benchmark was run on 22 July 2026
and the 300-item morphology benchmark on 24 July 2026. The fine-tuning study
additionally uses the open models \texttt{Qwen2.5-0.5B-Instruct} and
\texttt{Qwen2.5-3B-Instruct} \cite{qwen2025}, in their 4-bit MLX conversions
(\texttt{mlx-community/Qwen2.5-0.5B-Instruct-4bit} and \texttt{Qwen2.5-3B-Instruct-4bit}).
Extending the evaluation to further frontier models (for example Gemini and Claude) to
show the dissociation is not specific to one provider is left to future work.
...
one-sided binomial test, overall and per model $\times$ category, and I report Wilson
...
a claim, so I built a second, morphology-only benchmark of 300 items whose stems are
...
I implement Kyrgyz nominal inflection as an explicit rule engine covering four-way
...
early version of my own rule, which the independent Apertium finite-state description
of Kyrgyz \cite{khanna2021apertium} confirmed, after which the rule was corrected. I
...
selected stems are common words rather than an arbitrary sample. I further screen out
...
I fine-tune with LoRA \cite{hu2022lora} on the rule-generated training pairs and
evaluate two ways: a
...
\begin{itemize}
  ...
  \item Multiple choice measures recognition, which is easier than free production;
        I therefore report generation separately in the fine-tuning study.
...
The benchmark, rule engine, and all code will be released publicly upon acceptance.
A repository link is omitted here to preserve anonymity for double-blind review.
